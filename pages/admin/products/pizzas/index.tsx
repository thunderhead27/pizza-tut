...
export default function AdminPizzas({ pizzas }: InferGetServerSidePropsType<typeof getServerSideProps>) {
    ...
    let price = [{ size: "small", price: 0 }, { size: "medium", price: 0 }, { size: "large", price: 0 }, { size: "xLarge", price: 0 }]

    const priceArray = price.map((price) => price.price.toString());
    ...
    const submitHandler: SubmitHandler<FormValues> = async (data) => {
        ...
        axios.post('/api/admin/products/create', newPizza);
                    ...
                    <div>
                        ...
                        <div className="flex flex-col items-center gap-y-4">
                                    ...
                                    <tbody>
                                        {pizzas.map(el => (
                                            <tr key={el._id}>
                                                ...
                                                <td className="px-8">{el.name}</td>
                                                <td>{el.description}</td>
                                                <td>{el.price.map((el2, i) => <div className="uppercase" key={i}>{el2.size}: ${el2.price}</div>)}</td>
                                                <td className="px-8">{el.points}</td>
                                                <td><button className="bg-cOrange py-2 px-4 rounded-md" onClick={() => router.push(`/admin/products/pizzas/${el._id}`)}>Edit</button></td>
                                                <td className="align-middle px-6">
                            ...
                            <Create visible={active === 'create'}>
                                <h2 className="text-xl font-bold">CREATE NEW PIZZA</h2>
...
AdminPizzas.auth = { adminOnly: true };